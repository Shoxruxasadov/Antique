...
const ANALYSIS_FETCH_TIMEOUT_MS = 60000;
...
/** Tarmoq/ulanish xatolarini bitta foydalanuvchiga tushunarli xabarga aylantiradi */
function normalizeNetworkError(e) {
  ...
  if (msg.includes('network request failed') || msg.includes('failed to fetch') || msg.includes('load failed') ||
      msg.includes('network error') || msg.includes('econnrefused') || msg.includes('enotfound') ||
      msg.includes('etimedout') || msg.includes('econnreset') || e?.name === 'AbortError') {
...
async function getGeminiApiKey() {
  ...
  try {
    ...
    if (e?.name === 'AbortError') throw new Error('Connection error. Please check your internet and try again.');
...
- description: string (minimum 2-3 full sentences: describe appearance, materials, period style, and notable details; avoid single short lines)
...
- rarity_score: number (1-10; 10 = extremely rare/unique, 1 = very common; base on scarcity, demand, and collectibility)
- overall_condition_summary: string (exactly one of: "Excellent", "Very Good", "Good", "Fair", "Poor", "Damaged" — overall physical condition of the item)
- condition_grade: string (optional, e.g. "B+", "A"; for internal use)
- condition_description: string (1-2 sentences)
- category: string (exactly one of: "Coin", "Timepiece", "Painting", "Textile", "Ceramics", "Furniture", "Jewelry", "Sculpture", "Coffee Pot", "Silverware", "Tableware", "Metalware", "Vessel", "Pottery" — prefer the specific type, e.g. "Coffee Pot" for coffee pots, "Silverware" for silver tableware; use "Other" only if none of these fit)
...
- period: string (e.g. "Late 19th – Early 20th Century", "Georgian", "Victorian")
- origin: string (place of origin, e.g. "France", "England")
- maker: string (manufacturer, workshop, or "Unknown")
- material: string (e.g. "Sterling Silver", "Oak", "Porcelain")
- dimensions: string (e.g. "Approx. 10-12 inches height", "5 cm diameter")
- estimated_age_display: string (e.g. "c. 120 years", "Early 20th century")
- rarity_label: string (exactly one of: "Common", "Uncommon", "Rare", "Very Rare", "Extremely Rare")
- market_demand: string (exactly one of: "Low", "Medium", "High")
- origin_provenance: string (minimum 2-3 full sentences: where and how the item was likely produced, region, period context, and typical use or trade; avoid single short lines)
- region_of_origin: string (more specific location, e.g. "Rome, Italy" or "Paris, France" — not just "Roman Empire")
- typical_use: string (exactly one of: "circulation", "decoration", "ceremony" — primary use of the item)
- surface_condition: string (exactly one of: "Pristine", "Minor Wear", "Moderate Wear", "Heavy Wear", "Damaged")
- structural_integrity: string (exactly one of: "Intact", "Stable", "Minor Issues", "Compromised", "Broken")
- age_wear: string (exactly one of: "Minimal", "Light Patina", "Moderate Aging", "Heavy Aging", "Severe Deterioration")
- authenticity_markers: string (exactly one of: "Strong", "Consistent", "Partial", "Weak", "Inconclusive")
- authenticity_confidence_level: string (exactly one of: "High", "Medium", "Low" — overall authenticity confidence)
- authenticity_material_match: boolean (true if materials are consistent with period/origin)
- authenticity_style_match: boolean (true if style is consistent with period)
- authenticity_wear_pattern: boolean (true if wear is consistent with stated age)
- authenticity_known_red_flags: string (short description of any red flags: modern materials, wrong proportions, etc.; or empty string if none)
- care_tips: string (2-4 sentences: item-specific care advice, e.g. "Do not clean or polish; store in a dry place.")`;
...
function getAnalysisPrompt(locale) {
  ...
  return `${PROMPT}\n\nIMPORTANT — Output language: ALL user-visible text/string values MUST be written in ${lang}. The app displays these in the user's selected language. Include: name, description, not_antique_reason, overall_condition_summary, condition_description, category, period, origin, maker, material, dimensions, estimated_age_display, rarity_label, market_demand, origin_provenance, region_of_origin, typical_use, surface_condition, structural_integrity, age_wear, authenticity_markers, authenticity_confidence_level, care_tips, and any other specification or label fields. Only search_keywords must stay in English (for eBay search).`;
}
