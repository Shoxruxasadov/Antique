...
export async function fetchMarketPricesFromEbay(keywords = []) {
  ...
  if (isSupabaseConfigured() && supabase) {
    ...
    try {
      ...
      if (error) {
        ...
        if (data != null) console.log('[eBay search] response body:', JSON.stringify(data, null, 2));
      ...
      return {
        ...
        market_value_median: Number(data?.market_value_median) ?? 0,
      ...
      if (e?.context?.body) console.log('[eBay search] error body:', e.context.body);
    }
  }
  ...
  return {
    ...
    market_value_median: (mockMin + mockMax) / 2,
