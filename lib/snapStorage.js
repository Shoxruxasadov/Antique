 ...
 * Path: snaps/{userId}/{snapId}.jpg (userId 'anonymous' bo‘lsa login qilmagan user)
 * @param {string} userId - auth user id yoki 'anonymous'
...
export async function uploadSnapImage(userId, snapId, base64Data, contentType = 'image/jpeg') {
  ...
  try {
    ...
    console.warn('[Snap upload] failed:', msg);
    if (userId === 'anonymous' && (msg.includes('policy') || msg.includes('row-level security') || msg.includes('403'))) {
      console.warn('[Snap upload] Login qilmagan user uchun Storage policy kerak: supabase/storage_anonymous_snaps_policy.sql ni Dashboard → SQL Editor da ishga tushiring.');
    }
    return null;
  }
}
