...
export async function syncLocalCollectionToSupabase(userId) {
  ...
  for (const snap of history) {
    ...
    // antiques.image_url NOT NULL — fallback so insert never fails
    const antiqueImageUrl = publicImageUrl || imageUrl || '';
    const antiqueRow = {
      user_id: userId,
      ...
      rarity_score: ant.rarity_score ?? 0,
      ...
      category: Array.isArray(ant.category) ? ant.category : (ant.category != null && ant.category !== '' ? [String(ant.category)] : ['Antique']),
